...
library ClaimTypes {
    ...
    bytes32 internal constant CLAIM_HASH = 0xcabfb0e3a8d7a66441f25d7c0c8fd5180bf05942b71e56f775653ded5be5d5ca;
    ...
    function hash(Claim calldata order) internal pure returns (bytes32) {
        return keccak256(abi.encode(CLAIM_HASH, order.nonce, order.deadline, order.user, keccak256(abi.encodePacked(order.amounts)), keccak256(abi.encodePacked(order.tokens))));
    }
}
